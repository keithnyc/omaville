...
for(let i=0;i<100;i++) assert(!T.spawn(data,size,roads,[],colors).police);
for(let tier=0;tier<3;tier++) {
  data[190]='S'+tier;
  let count=0;
  for(let i=0;i<400;i++) {
    const car=T.spawn(data,size,roads,[],colors);
    if(car.police) {count++;assert(T.nearService(data,size,car.tile,'S'));assert(!car.schoolBus);}
  }
  assert(count>5 && count<200);
}
const cruiser={tile:210,prev:209,next:211,path:T.makePath(size,210,209,211),distance:.1,
  speed:0,cruise:.6,age:1,police:true,dispatchOffset:0};
...
data[190]='_0';
assert(!T.update(pool,33,data,size,2,roads,colors).some(c=>c.police));
console.log('PASS: station-gated spawning at all tiers, no bus hybrids, alternating lights, emergency speed, safe following and last-station removal.');
